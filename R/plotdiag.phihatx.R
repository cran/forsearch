     ...
     verbose=TRUE)
{
     ...
     if(verbose) {
          print("", quote = FALSE)
          print("Running plotdiag.phihatx", quote = FALSE)
          print("", quote = FALSE)
...
#                 if(diagnose)Hmisc::prn(as.character(out))   
     ...
     prepstuff <- function(rightforn,gg){
          df1 <- rightforn$"PhiHat"
          if(length(c(df1))==1) stop("phiHat was not estimated")
          df2 <- df1
          ndf2 <- length(df2)
          column1 <- 1:ndf2
          for(ir in 1:ndf2){
...
##################################################   Main function    ##############################################
     ...
     if(verbose) {
          print("", quote = FALSE)
          print("Finished running plotdiag.phihatx", quote = FALSE)
          print("", quote = FALSE)
          print(date(), quote = FALSE)
          print("", quote = FALSE)
     }
